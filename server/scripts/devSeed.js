...
async function main() {
  ...
  const Brand = models.Brand || (models.models && models.models.Brand);
  const PhoneModel = models.PhoneModel || (models.models && models.models.PhoneModel);
  const RepairOption = models.RepairOption || (models.models && models.models.RepairOption);
  const FixItBooking =
    models.FixItBooking || (models.models && models.models.FixItBooking);

  if (!Device || !RepairService || !Booking || !Brand || !PhoneModel || !RepairOption || !FixItBooking) {
    throw new Error(
      "Could not locate Device/RepairService/Booking/FixIt models from server/src/db/models/index.js. " +
  ...
  const summary = {
    ...
    bookingsCreated: 0,
    bookingsExisting: 0,
    fixitBrandsCreated: 0,
    fixitBrandsExisting: 0,
    fixitModelsCreated: 0,
    fixitModelsExisting: 0,
    fixitRepairsCreated: 0,
    fixitRepairsExisting: 0,
  };
  ...
  for (const brand of FIXIT_BRANDS) {
    const [brandRow, created] = await Brand.findOrCreate({
      where: { id: brand.id },
      defaults: { id: brand.id, name: brand.name, svg: brand.svg },
    });
    if (created) summary.fixitBrandsCreated++;
    else summary.fixitBrandsExisting++;

    const modelsList = FIXIT_CATALOG[brand.id] || [];
    for (const model of modelsList) {
      const [modelRow, modelCreated] = await PhoneModel.findOrCreate({
        where: { brandId: brandRow.id, name: model.name },
        defaults: { brandId: brandRow.id, name: model.name },
      });
      if (modelCreated) summary.fixitModelsCreated++;
      else summary.fixitModelsExisting++;

      for (const repair of model.repairs) {
        const [repairRow, repairCreated] = await RepairOption.findOrCreate({
          where: {
            modelId: modelRow.id,
            type: repair.type,
            name: repair.name,
        ...
        if (repairCreated) summary.fixitRepairsCreated++;
        else summary.fixitRepairsExisting++;
      }
    }
  }
